...
class DbExecutor(val kafkaConfigurator: KafkaConfigurator) {
  ...
  // chats
  //
  //  def createSimpleChat(userId: UUID)(implicit connection: Connection) : DbResponse[Chat] = {
  ...
  //
  //
  //  def createChat(users: List[User], chatName: ChatName): DbResponse[Chat] = {
  //    val listSize = users.length
  //    if (listSize < 2)  Left(QueryError(ERROR, AtLeastTwoUsers))
  //    else {
  //
  //    }
