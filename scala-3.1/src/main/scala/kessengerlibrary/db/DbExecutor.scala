...
class DbExecutor(val kafkaConfigurator: KafkaConfigurator) {
    ...
    Using(connection.prepareStatement(sql)) {
      ...
      case Failure(ex) =>
        ...
        if (ex.getMessage.contains("duplicate key value violates unique constraint")) {
          Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.LoginTaken(ErrorCode(6))))
        }
        ...
        if (a == 1) {
          ...
          Using(connection.prepareStatement( sql2 )) {
            ...
            case Failure(ex) =>
              ...
              connection.setAutoCommit(true)
              if (ex.getMessage.contains("duplicate key value violates unique constraint")) {
                Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.DataProcessingError(ErrorCode(7))))
              }
            ...
            case Success(value) =>
              if (value == 1) {
                ...
                connection.rollback(beforeAnyInsertions)
                connection.setAutoCommit(true)
                Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.DataProcessingError(ErrorCode(8))))
              }
          }
        } else {
          connection.rollback(beforeAnyInsertions)
          connection.setAutoCommit(true)
          Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.DataProcessingError(ErrorCode(9))))
        }
    }
  }
  ...
  def updateMyLogin(userId: UUID, newLogin: Login)(implicit connection: Connection): DbResponse[Int] = {
    Using(connection.prepareStatement("UPDATE users SET login = ? WHERE user_id = ? ")) { //    AND login = ?
      ...
      case Failure(ex) =>
        if (ex.getMessage.contains("duplicate key value violates unique constraint")) {
          Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.LoginTaken(ErrorCode(10))))
        }
        handleExceptionMessage(ex)
        ...
        Using(statement.executeQuery()) {
          (resultSet: ResultSet) =>
            if (resultSet.next()) {
              ...
              Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.IncorrectLoginOrPassword(ErrorCode(11))))
        } match {
  ...
  /**
   * jeśli zwraca zero to znaczy, że nie ma updatowanej sesji i takie
   * rządanie należy przekierować na stronę logowania
   *
   * @param sessionId
   * @param userId
   * @param newValidityTime
   * @return
   */
  ...
  /**
   * jeśli zwróci zero to znaczy, że nie usunęło sesji bo jej nie ma,
   * albo bo usera już nie ma
   * wtedy przekieruj na stronę logowania.
   *
  ...
  def createSingleChat(me: User, otherId: UUID, chatId: ChatId, chatName: ChatName )(implicit connection: Connection, ec: ExecutionContext): DbResponse[Int] = {
    checkDuplicatedChat(me.userId, otherId) match {
      ...
      case Right(value) =>
        if ( value > 0 ) Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.UnsupportedOperation(ErrorCode(12))))
        else {
          connection.setAutoCommit(false)
          val beforeAnyInsertions: Savepoint = connection.setSavepoint()
          ...
          if (a == 3) {
            ...
            connection.rollback( beforeAnyInsertions )
            connection.setAutoCommit(true)
            Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.DataProcessingError(ErrorCode(13))))
          }
        }
    }
  }
  ...
  def createGroupChat(users: List[User], chatName: ChatName, chatId: ChatId)(implicit connection: Connection, ex: ExecutionContext): DbResponse[Chat] = {
    ...
    if (listSize < 2)
      Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.AtLeastTwoUsers(ErrorCode(14))))
    else {
      connection.setAutoCommit(false)
      val beforeAnyInsertions: Savepoint = connection.setSavepoint()
        ...
        case Success(v) =>
          if (v == 1) {
            ...
            if (totalAffectedRows == users.length) {
              ...
              connection.rollback(beforeAnyInsertions)
              connection.setAutoCommit(true)
              Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.DataProcessingError(ErrorCode(15))))
            }
          } else {
            connection.rollback(beforeAnyInsertions)
            connection.setAutoCommit(true)
            Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.DataProcessingError(ErrorCode(16))))
          }
      }
    }
  }
  ...
  def addNewUsersToChat(users: List[UUID], chatId: String, chatName: ChatName)(implicit connection: Connection, ec: ExecutionContext): DbResponse[Int] = {
    if (users.isEmpty) Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.NoUserSelected(ErrorCode(17))))
    else {
    ...
    if (groupChat) {
      numOfChatUsers(chatId) match {
        ...
        case Right(chatUsers) =>
          if (chatUsers < 0)
            Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.DataProcessingError(ErrorCode(19))))
          else if (chatUsers == 0)
            Right(chatUsers)
            ...
            Using(connection.prepareStatement( sql )) {
              ...
              case Success(value) =>
                ...
                else Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.DataProcessingError(ErrorCode(20))))
            }
          }
      }
    } else Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.UnsupportedOperation(ErrorCode(21))))
  }
  ...
  private def handleExceptionMessage[A](ex: Throwable): DbResponse[A] = {
    if (ex.getMessage == "FATAL: terminating connection due to administrator command"
      ...
      Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.NoDbConnection(ErrorCode(1))))
    }
    else if (ex.getMessage.toLowerCase.contains("timeout")) {
      Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.TimeOutDBError(ErrorCode(2))))
    }
    else if (ex.getMessage == "Incorrect login or password") {
      Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.IncorrectLoginOrPassword(ErrorCode(3))))
    }
      ...
      Left(QueryError(QueryErrorType.ERROR, QueryErrorMessage.DataProcessingError(ErrorCode(4))))
    }
    else
      Left(QueryError(QueryErrorType.ERROR,QueryErrorMessage.UndefinedError(ErrorCode(5),ex.getMessage)))
  }
