...
object WindowedAvgServerDelay {
  ...
  given encoder : Encoder[WindowedAvgServerDelay] = (a: WindowedAvgServerDelay) => {
    Json.obj(
  ...
  given decoder: Decoder[WindowedAvgServerDelay] = (c: HCursor) => {
    for {
  ...
  def nullMessage: WindowedAvgServerDelay = WindowedAvgServerDelay(null, null, 0L)
